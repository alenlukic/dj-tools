    ...
    for track_id in track_ids:
        ...
        for at in artist_tracks:
            ...
            try:
                ...
                deletion_statuses[str((track_id, artist_id))] = (
                    DBUpdateType.DELETE.value
                )
            except Exception as e:
                handle(e)
                deletion_statuses[str((track_id, artist_id))] = (
...
def delete_feature_values(session, track_ids):
    ...
    for track_id in track_ids:
        feature_values = (
            session.query(FeatureValue).filter_by(track_id=track_id).first()
        )

        try:
            deletion_statuses[str(track_id)] = _get_deletion_status(
...
def delete_transition_matches(session, track_ids):
    ...
    for on_deck_id, candidate_id in deletion_queue:
        match = (
            session.query(TransitionMatch)
            .filter_by(on_deck_id=on_deck_id, candidate_id=candidate_id)
            .first()
        )

        try:
            deletion_statuses[str((on_deck_id, candidate_id))] = _get_deletion_status(
                session, match
            )
        except Exception as e:
            handle(e)
            deletion_statuses[str((on_deck_id, candidate_id))] = (
                DBUpdateType.FAILURE.value
            )
            continue
    ...
    for track in tracks:
        ...
        try:
            ...
            for field in COMMENT_FIELDS:
                ...
                if (
                    col_value is None and comment_value is None
                ...
                if field == TrackDBCols.BPM.value or field == TrackDBCols.ENERGY.value:
                    ...
                    comment_value = (
                        None if comment_value is None else int(comment_value)
                    )
                ...
                if field == TrackDBCols.TITLE.value:
                    ...
                    if title != col_value or title != comment_value:
                        ...
                        log_buffer.append(
                            update_msg
                            % ("column", field, "deduped", str(col_value), str(title))
                        )
                ...
                if col_value is not None:
                    ...
                    log_buffer.append(
                        update_msg
                        % ("column", field, "comment", str(None), str(comment_value))
                    )
            ...
            if len(log_buffer) > 0:
                ...
                tags_to_update = {
                    k: v for k, v in tags_to_update.items() if k in ID3_COMMENT_FIELDS
                }
...
def _get_deletion_status(session, entity):
    return (
        DBUpdateType.DELETE.value
        if session.safe_delete(entity) is True
