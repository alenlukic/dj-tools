...
class AudioFile:
    ...
    def generate_comment(self, metadata):
        comment = {
            ...
            if not is_empty(v) and k != TrackDBCols.COMMENT.value
        }
    ...
    def generate_title(self, camelot_code, key, bpm):
        ...
        artist_midfix = self.format_artist_string(featured) + (
            "" if featured is None else " ft. " + featured
        )
    ...
    def format_artist_string(self, featured):
        ...
        filtered_artists = [
            transform_artist(a)
            for a in split_artist_string(artists)
            if a not in featured_set
        ]
        ...
        separator = (
            " and " if any("&" in artist for artist in filtered_artists) else " & "
        )
    ...
    def read_tags(self):
        return {
            k: getattr(self.get_id3().get(k, {}), "text", []) for k in TRACK_MD_ID3_TAGS
        }
    ...
    def write_tag(self, tag, value, save=True):
        ...
        if tag in id3.keys():
            ...
            if tag == ID3Tag.TITLE.value:
                ...
                or tag == ID3Tag.COMMENT_XXX.value
            ):
