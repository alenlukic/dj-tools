...
class FinalRecordFactory(TagRecordFactory):
    ...
    def update_row(self):
        ...
        self.row = {
            ...
            'bpm': FinalRecordFactory._get_final_bpm(initial_record, post_mik_record, post_rb_record),
            'key': FinalRecordFactory._get_final_key(initial_record, post_mik_record, post_rb_record),
    ...
    def _get_final_key(initial_record, mik_record, rb_record):
        initial_record_key = CANONICAL_KEY_MAP.get(initial_record.key.lower()).capitalize() if initial_record.key is not None else ''
