...
class GoogleDrive:
    def __init__(self):
        ...
        self.restore_dir = join_config_paths(
            [["DATA", "ROOT"], ["DATA", "BACKUP_RESTORE_MUSIC_DIR"]]
        )
    ...
    def _get_paged_files(self, next_page_token, query_args, prev_file_count=0):
        ...
        while cur_token is not None:
            ...
            print_and_log(
                "Retrieved %d files (%d total)" % (num_results, total_files), info
            )
    ...
    def get_target_revisions(self, gd_resources, query_args, latest_date):
        for gd_resource in gd_resources:
            try:
                ...
                results = (
                    self.drive.revisions().list(**args).execute().get("revisions", [])
                )

                revs = [
                    {
                        "id": r["id"],
                        "modifiedTime": datetime.strptime(r["modifiedTime"], TS_FORMAT),
                    }
                    for r in results
                ]
                revs = sorted(
                    [r for r in revs if r["modifiedTime"] <= latest_date],
