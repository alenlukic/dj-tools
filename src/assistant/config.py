...
from src.assistant.command import Command
from src.assistant.command_argument import CommandArgument
...
ALL_ALIASES = (
    {EXIT, MATCH, RELOAD, INGEST}.union(EXIT_ALIASES)
    .union(MATCH_ALIASES)
    .union(RELOAD_ALIASES)
)
...
COMMANDS = {
    ...
    MATCH: Command(
        MATCH,
        "Find harmonic mixing matches for the given BPM and Camelot code.",
        "print_transition_matches",
        MATCH_ALIASES,
        [
            CommandArgument(
                "track_title",
    ...
    RELOAD: Command(
        RELOAD, "Reload processed tracks.", "reload_track_data", RELOAD_ALIASES
    ),
    INGEST: Command(
        INGEST,
        ...
        [
            CommandArgument(
                "upsert", "boolean", "Indicates whether to upsert.", 0, "False", False
            )
        ],
    ),
}
